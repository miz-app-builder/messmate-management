-- MessMate Bazar RPC repair
-- Ensures the atomic create RPC exists after unit_price compatibility was applied.
...
begin
  ...
  select coalesce(bazar_approval_required, true)
  ...
  values(
    ...
    case when v_approval_required then null else now() end,
    case when v_approval_required then null else auth.uid() end
  )
